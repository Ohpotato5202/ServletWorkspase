...
public class AjaxJson extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/* 
	       ...
	       *  - 비동기 통신에서 자주 사용되는 포맷
	       *  - 순수 문자열로 이루어져 있어서 가독성이 좋고, 가볍기 때문에 플랫폼간의 데이터교환에 자주 사용된다. 
	       * 
	       * */
	      //클래스없이 json객체만들기
...
//	      String jsonMember = "{\"id\":\""+m.getId()+"\",\"name\":\""+m.getName()+"\",\"profile\":\""+m.getProfile()+"\"}";
...
//	      obj.<String, String> put("name", m.getName()); // '{"name":"김고은" }'
...
//	      response.getWriter().print(obj);
	      ...
	      // 검색어와 일치하는 ID가 존재하는경우 일치하는 회원정보를 보관
	      Member member = null;
